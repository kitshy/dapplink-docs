...
export async function init(root: string | undefined) {
  intro(bold(cyan('Welcome to DappLink!')))
  ...
  const options: ScaffoldOptions = await group(
    {
      root: async () => {
        ...
        return text({
          message: 'Where should DappLink initialize the config?',
          initialValue: './',
          validate(value) {
      ...
      injectNpmScripts: () =>
        confirm({
          message: 'Add DappLink npm scripts to package.json?'
        })
    },
    {
      onCancel: () => {
...
export function scaffold({
  ...
  const renderFile = (file: string) => {
    ...
    if (useMjs && file === '.dapplink-docs/config.js') {
  ...
  const filesToScaffold = [
    ...
    '.dapplink-docs/config.js'
  ]

  if (theme === ScaffoldThemeType.DefaultCustom) {
    filesToScaffold.push(
      '.dapplink-docs/theme/index.js',
      '.dapplink-docs/theme/style.css'
    )
  } else if (theme === ScaffoldThemeType.Custom) {
    filesToScaffold.push(
      '.dapplink-docs/theme/index.js',
      '.dapplink-docs/theme/style.css',
      '.dapplink-docs/theme/Layout.vue'
    )
  }
  ...
  const gitignorePrefix = dir ? `${dir}/.vitepress` : '.dapplink-docs'
