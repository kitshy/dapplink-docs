...
const { localeIndex, theme } = dappLinkData
...
const filterText = disableQueryPersistence.value
  ? ref('')
  : useSessionStorage('dapplink-docs:local-search-filter', '')

const showDetailedList = useLocalStorage(
  'dapplink-docs:local-search-detailed-list',
  theme.value.search?.provider === 'local' &&
  ...
  async ([index, filterTextValue, showDetailedListValue], old, onCleanup) => {
    ...
    for (const { id, mod } of mods) {
      ...
      if (comp?.render || comp?.setup) {
        ...
        app.provide(dataSymbol, dappLinkData)
        Object.defineProperties(app.config.globalProperties, {
          $frontmatter: {
            get() {
              return dappLinkData.frontmatter.value
            }
          },
          $params: {
            get() {
              return dappLinkData.page.value.params
            }
          }
        })
